...
TEST(TestLoopBuffer, test_uint8_t) {
  {
    infinite_arrary_uint8_t loopbuff;
    uint8_t buffer[20] = {0};
    infinite_arrary_init_uint8_t(&loopbuff, buffer, ARRARY_SIZE(buffer));

    EXPECT_TRUE(infinite_arrary_get_useful_start_offset_uint8_t(&loopbuff) < 0);
    EXPECT_TRUE(infinite_arrary_get_useful_end_offset_uint8_t(&loopbuff) < 0);
...

  {
    ...
    for (unsigned i = 0; i < 40; ++i) {
      ...
      if (i < 20) {
        EXPECT_EQ(infinite_arrary_get_useful_start_offset_uint8_t(&loopbuff),
                  (int32_t)0);
        EXPECT_EQ(infinite_arrary_get_useful_end_offset_uint8_t(&loopbuff),
                  (int32_t)i);
      } else {
        EXPECT_EQ(infinite_arrary_get_useful_start_offset_uint8_t(&loopbuff),
                  (int32_t)i - 19);
...
TEST(TestLoopBuffer, test_uint32_t) {
  {
    infinite_arrary_uint32_t loopbuff;
    uint32_t buffer[20] = {0};
    infinite_arrary_init_uint32_t(&loopbuff, buffer, ARRARY_SIZE(buffer));

    EXPECT_TRUE(infinite_arrary_get_useful_start_offset_uint32_t(&loopbuff) <
                0);
    EXPECT_TRUE(infinite_arrary_get_useful_end_offset_uint32_t(&loopbuff) < 0);
...

  {
    ...
    for (unsigned i = 0; i < 40; ++i) {
      ...
      if (i < 20) {
        EXPECT_EQ(infinite_arrary_get_useful_start_offset_uint32_t(&loopbuff),
                  (int32_t)0);
        EXPECT_EQ(infinite_arrary_get_useful_end_offset_uint32_t(&loopbuff),
                  (int32_t)i);
      } else {
        EXPECT_EQ(infinite_arrary_get_useful_start_offset_uint32_t(&loopbuff),
                  (int32_t)i - 19);
